...
main(int argc, const char *argv[])
{
    ...
    size_t read_size;
    lxb_codepoint_t cp;
    lxb_encoding_encode_t ctx_encode = {0};
    lxb_encoding_decode_t ctx_decode = {0};
    ...
    do {
        ...
        while (data < end) {
            /* Decode */
            cp = from->decode(&ctx_decode, &data, end);
            ...
            len = to->encode(&ctx_encode, &out, out_end, cp);
    ...
    /*
     * For LXB_ENCODING_ISO_2022_JP we must call
     * lxb_encoding_encode_iso_2022_jp_eof function after encode processing.
     */
    if (to->encoding == LXB_ENCODING_ISO_2022_JP) {
        out = outbuf;

        len = lxb_encoding_encode_iso_2022_jp_eof(&ctx_encode, &out, out_end);

        if (len != 0) {
            printf("%.*s", (int) len, (char *) outbuf);
        }
    }
