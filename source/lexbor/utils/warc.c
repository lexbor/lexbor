...
#include "lexbor/core/conv.h"
    ...
    #define LXB_UTILS_WARC_MAX_HEADER_NAME 4096
#endif

#ifndef LXB_UTILS_WARC_MAX_HEADER_VALUE
    #define LXB_UTILS_WARC_MAX_HEADER_VALUE 4096 * 4
#endif
...
lxb_utils_warc_parse_file(lxb_utils_warc_t *warc, FILE *fh)
{
    ...
    do {
        ...
        size = fread(buffer, sizeof(lxb_char_t), buffer_size, fh);
        if (size != buffer_size) {
            if (feof(fh)) {
        ...
        status = lxb_utils_warc_parse(warc, &buf_ref, (buffer + buffer_size));
    }
