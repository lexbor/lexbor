...
lxb_font_table_prep_t *
lxb_font_table_prep(lxb_font_t *mf, uint8_t* font_data, size_t size)
{
    lxb_font_table_prep_t *table;
    uint8_t *data;
    uint32_t offset;
    uint32_t length;
    uint32_t i;

    table = LXB_FONT_ALLOC(1, lxb_font_table_prep_t);
    if (table == NULL) {
        return lxb_font_failed(mf, LXB_STATUS_ERROR_MEMORY_ALLOCATION);
    }

    offset = mf->cache.tables_offset[LXB_FONT_TKEY_PREP];
    length = mf->cache.tables_length[LXB_FONT_TKEY_PREP];
