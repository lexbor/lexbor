...
{
    lxb_status_t status = lxb_tag_heap_init(NULL, 32);
...
TEST_BEGIN(init_args)
{
    ...
    lxb_status_t status = lxb_tag_heap_init(&tag_heap, 0);
...
TEST_BEGIN(clean)
{
    ...
    lxb_tag_heap_t *tag_heap = lxb_tag_heap_create();
    test_eq(lxb_tag_heap_init(tag_heap, 32), LXB_STATUS_OK);
...
TEST_BEGIN(name_by_id)
{
    ...
    name = lxb_tag_name_by_id(tag_heap, LXB_TAG_DIV, &len);
    test_eq_u_str(name, (const lxb_char_t *) "div");
    test_eq_size(len, 3UL);

    name = lxb_tag_name_by_id(tag_heap, LXB_TAG__LAST_ENTRY + 10, &len);
    test_eq(name, NULL);
    test_eq_size(len, 0UL);

    name = lxb_tag_name_by_id(tag_heap, LXB_TAG__LAST_ENTRY, &len);
    test_eq(name, NULL);
    test_eq_size(len, 0UL);
...
TEST_BEGIN(name_by_id_without_len)
{
    ...
    name = lxb_tag_name_by_id(tag_heap, LXB_TAG_DIV, NULL);
    test_eq_u_str(name, (const lxb_char_t *) "div");

    name = lxb_tag_name_by_id(tag_heap, LXB_TAG__LAST_ENTRY + 10, NULL);
    test_eq(name, NULL);

    name = lxb_tag_name_by_id(tag_heap, LXB_TAG__LAST_ENTRY, NULL);
    test_eq(name, NULL);
...
TEST_BEGIN(destroy)
{
    ...
    test_eq(lxb_tag_heap_destroy(tag_heap), NULL);
}
TEST_END
