...
{
    lxb_status_t status = lxb_html_tag_heap_init(NULL, 32);
...
TEST_BEGIN(init_args)
{
    ...
    lxb_status_t status = lxb_html_tag_heap_init(&tag_heap, 0);
...
TEST_BEGIN(init_stack)
{
    ...
    status = lxb_html_tag_heap_init(&tag_heap, 32);
    test_eq(status, LXB_STATUS_OK);
...
TEST_BEGIN(clean)
{
    ...
    tag_data = lxb_html_tag_append(&tag_heap, (const lxb_char_t *) "best_tag", 8);
...
TEST_BEGIN(data_by_id)
{
    ...
    tag_data = lxb_html_tag_data_by_id(&tag_heap, LXB_HTML_TAG_DIV);
    test_ne(tag_data, NULL);

    tag_data = lxb_html_tag_data_by_id(&tag_heap, LXB_HTML_TAG__LAST_ENTRY + 100);
...
TEST_BEGIN(data_by_name)
{
    ...
    tag_data = lxb_html_tag_data_by_name(&tag_heap,
...
TEST_BEGIN(id_by_name)
{
    ...
    tag_id = lxb_html_tag_id_by_name(&tag_heap, (const lxb_char_t *) "div", 3);
    test_eq(tag_id, LXB_HTML_TAG_DIV);

    tag_id = lxb_html_tag_id_by_name(&tag_heap, (const lxb_char_t *) "zzz", 3);
    test_eq(tag_id, LXB_HTML_TAG__UNDEF);
...
TEST_BEGIN(append)
{
    ...
    for (size_t i = 0; i < 10000; i++) {
        ...
        tag_data = lxb_html_tag_append(&tag_heap,
    ...
    for (size_t i = 0; i < 10000; i++) {
        ...
        tag_id = lxb_html_tag_id_by_name(&tag_heap,
                                         (const lxb_char_t *) tmp, (size_t) n);
...
TEST_BEGIN(destroy)
{
    ...
    test_eq(lxb_html_tag_heap_destroy(tag_heap, true), NULL);

    tag_heap = lxb_html_tag_heap_create();
    test_eq(lxb_html_tag_heap_init(tag_heap, 32), LXB_STATUS_OK);

    test_eq(lxb_html_tag_heap_destroy(tag_heap, false), tag_heap);
    test_eq(lxb_html_tag_heap_destroy(NULL, false), NULL);
}
TEST_END
...
{
    lxb_html_tag_heap_t tag_heap;
    lxb_html_tag_heap_init(&tag_heap, 32);
